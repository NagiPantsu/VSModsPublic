...
public static class ModSystemWearableStatsPatch
{
        ...
        ref ICoreAPI ___api, ref Dictionary<int, EnumCharacterDressType[]> ___clothingDamageTargetsByAttackTacket, ref AssetLocation ___ripSound)
    {
        ...
        if (itemslot.Empty || !TryGetWearableStats(itemslot, out var protectionModifiers) || itemslot.Itemstack.Collectible.GetRemainingDurability(itemslot.Itemstack) <= 0)
        {
            ...
            if (!slot.Empty)
            {
                ...
                var damageAmount = Math.Max(1, GameMath.RoundRandom(___api.World.Rand, damage * num3 / 4f));
                if (damageAmount > 0)
                {
                    ___api.World.PlaySoundAt(___ripSound, player.Entity);
                    slot.Itemstack?.Collectible.DamageItem(___api.World, player.Entity, slot, damageAmount);
                }
            }
        ...
        var flatDamageReduction = protectionModifiers.FlatDamageReduction *
                                  Attribute.ArmorFlatDamageReductionMultiplier.GetFloat(itemslot.Itemstack, 1f);
        ...
        for (var index = 1; index <= damageTier; ++index)
        {
            ...
            num5 /= Attribute.ArmorPerTierFlatDamageProtectionLossMultiplier.GetFloat(itemslot.Itemstack, 1f);
    ...
    private static bool TryGetWearableStats(ItemSlot itemSlot, out ProtectionModifiers protectionModifiers)
    {
        ...
        var supplier = itemSlot.Itemstack.Collectible.GetCollectibleInterface<IWearableStatsSupplier>();
        if (supplier == null || !supplier.IsArmorType(itemSlot))
            return false;

        protectionModifiers = supplier.GetProtectionModifiers(itemSlot);
        return protectionModifiers != null;
    }
        ...
        for (var index = 0; index < itemSlotArray.Length; ++index)
        {
            ...
            if (valueOrDefault && itemAttribute["protectionChance"][key1 + "-projectile"].Exists)
            {
                ...
                num4 = itemAttribute["projectileDamageAbsorption"].AsFloat(2f) *
                       Attribute.ShieldProjectileDamageAbsorptionMultiplier.GetFloat(itemslot.Itemstack, 1f);
            }
            else
            {
                num3 = itemAttribute["protectionChance"][key1].AsFloat();
                num4 = itemAttribute["damageAbsorption"].AsFloat(2f) *
                       Attribute.ShieldDamageAbsorptionMultiplier.GetFloat(itemslot.Itemstack, 1f);
            }
                    ...
                    : Math.Abs(GameMath.AngleRadDistance((float)pitch, (float)attackPitch)) < 0.5235987901687622)
            {
                ...
                if (player is IServerPlayer serverPlayer)
                {
                    ...
                    var message = Lang.Get("{0:0.#} of {1:0.#} damage blocked by shield ({2} use)", Math.Min(val1, damage), damage, key1);
