...
public static class Rarity
{
    ...
    private static RarityChancesConfig ChanceConfig => ModCore.Config.Chance; // tunable weights live here
    ...
    {
        // Use the sanitized/possibly user-tuned chances so we honor the new config file and run the clamp logic.
        var totalWeight = Config.Rarities.Values.Sum(i => ChanceConfig.GetEffectiveChance(i.Key, i.Weight));
        var randomValue = Random.Shared.NextDouble() * totalWeight;
        var cumulativeWeight = 0f;

        foreach (var item in Config.Rarities)
        {
            cumulativeWeight += ChanceConfig.GetEffectiveChance(item.Key, item.Value.Weight);
